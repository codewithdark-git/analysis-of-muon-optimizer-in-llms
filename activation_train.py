...
# Import our custom attention modules
from mhla_attention import create_attention_layer, MultiHeadLatentAttention, MultiHeadSelfAttention
...
class MoEModelConfig:
    ...
    attention_type: str = "mhsa"  # "mhsa" or "mhla"
    num_latents: int = 64  # Only used for MHLA
    
    # NEW: Activation specific parameters
    activation_type: str = "silu"  # "silu", "gelu", "relu", "tanh"

    def __post_init__(self):
        self.d_k = self.d_model // self.n_heads
        assert self.d_model % self.n_heads == 0, "d_model must be divisible by n_heads"
        assert self.attention_type.lower() in ["mhsa", "mhla"], "attention_type must be 'mhsa' or 'mhla'"
        assert self.activation_type.lower() in ["silu", "gelu", "relu", "tanh"], "activation_type must be 'silu', 'gelu', 'relu', or 'tanh'"
...
class Expert(nn.Module):
    ...
    def __init__(self, d_model: int, d_ff: int, dropout: float = 0.1, activation_type: str = "silu"):
        super().__init__()
...
class MixtureOfExperts(nn.Module):
    ...
    def __init__(
        ...
        dropout: float = 0.1,
        load_balancing_weight: float = 0.01,
        activation_type: str = "silu"  # NEW: Pass activation
    ):
        super().__init__()
        self.num_experts = num_experts
        self.top_k = top_k
        ...
        # Create experts with activation
        self.experts = nn.ModuleList([
            Expert(d_model, d_ff, dropout, activation_type) for _ in range(num_experts)
        ])
    ...
    def _compute_load_balancing_loss(
        ...
        tokens_per_expert = expert_mask.sum(dim=[0, 1, 2]) / (expert_mask.sum() + 1e-8)
...
class MoETransformerBlock(nn.Module):
    """Transformer block with MoE and configurable attention"""
    def __init__(
        self,
        d_model: int,
        n_heads: int,
        ...
        num_latents: int = 64,
        activation_type: str = "silu"  # NEW: Pass to MoE
    ):
        super().__init__()
        ...
        # MoE layer with activation
        self.feed_forward = MixtureOfExperts(
            d_model, d_ff, num_experts, top_k, dropout, activation_type=activation_type
        )
    ...
    def forward(self, x):
        # Self-attention (or latent attention)
...
class MoEMinimalLLM(nn.Module):
    """Minimal LLM with Mixture of Experts and configurable attention"""
    def __init__(self, config: MoEModelConfig):
        ...
        # Transformer blocks with MoE and configurable attention/activation
        self.transformer_blocks = nn.ModuleList([
            MoETransformerBlock(
                ...
                config.num_latents,
                config.activation_type  # NEW: Pass activation
            )
...
def train_moe_model(config: MoEModelConfig, train_loader: DataLoader, val_loader: DataLoader):
    ...
    print(f"   Attention type: {config.attention_type.upper()}")
    if config.attention_type.lower() == "mhla":
        print(f"   Latent tokens: {config.num_latents}")
    print(f"   Activation: {config.activation_type.upper()}")
    ...
    # Calculate active parameters (excluding experts not used in forward pass)
    active_params = total_params - expert_params + (expert_params * config.expert_top_k // config.num_experts)
    print(f"  📊 Active parameters per forward pass: {active_params:,}")
    ...
    model.train()
    step = 0
    pbar = tqdm(total=config.max_steps, desc=f"Training MoE+{config.attention_type.upper()}+{config.activation_type.upper()}")
...
def compare_combinations(config: MoEModelConfig, train_loader: DataLoader, val_loader: DataLoader):
    """Compare MHSA vs MHLA and activations with Muon"""
    results = {}
    activations = ["silu", "gelu", "relu", "tanh"]
    attentions = ["mhsa", "mhla"]
    
    for attention_type in attentions:
        for activation_type in activations:
            print(f"\n{'='*60}")
            print(f"🧪 TRAINING: MoE + {attention_type.upper()} + {activation_type.upper()}")
            print(f"{'='*60}")
            
            # Create config for this combination
            current_config = MoEModelConfig(
                ...
                activation_type=activation_type,  # NEW
            ...
            model, metrics = train_moe_model(current_config, train_loader, val_loader)
            training_time = time.time() - start_time
            
            # Store results
            results[f"{attention_type}-{activation_type}"] = {
                'model': model,
                'metrics': metrics,
            ...
            print(f"\n🎯 {attention_type.upper()}-{activation_type.upper()} Results:")
            print(f"⏱️ Training time: {training_time/60:.1f} minutes")
            print(f"📊 Final metrics: Loss={metrics['val_loss']:.4f}, "
                  f"Acc={metrics['val_accuracy']:.4f}, PPL={metrics['val_perplexity']:.2f}")
    
    # Print comparison
    print(f"\n{'='*60}")
    print(f"📈 COMPARISON: All Combinations with Muon")
    print(f"{'='*60}")
    ...
    for key in sorted(results):
        ...
        print(f"{attn:8} | {act:9} | {r['metrics']['val_loss']:7.2f} | {r['metrics']['val_perplexity']:6.1f} | {r['training_time']/60:9.1f} | {speedup:13.1f}x")
    
    # Best overall
    best_key = min(results, key=lambda k: results[k]['metrics']['val_loss'])
    print(f"\n🏆 Optimal: {best_key.upper()} (Loss: {results[best_key]['metrics']['val_loss']:.4f})")
    
    return results

if __name__ == "__main__":
    # Parse command line arguments
    parser = argparse.ArgumentParser(description="Train MoE model with configurable attention and activation")
    parser.add_argument("--attention", type=str, choices=["mhsa", "mhla", "both"], 
                       default="mhsa", help="Attention mechanism to use")
    parser.add_argument("--activation", type=str, choices=["silu", "gelu", "relu", "tanh", "both"], 
                       default="silu", help="Activation function to use")
    parser.add_argument("--num_latents", type=int, default=64, 
                       help="Number of latent tokens for MHLA")
    ...
    if args.attention == "both" or args.activation == "both":
        # Compare combinations
        print(f"\n📋 Comparing attention + activation mechanisms with Muon:")
        print(f"   Architecture: {config.d_model}d, {config.n_layers}L, {config.n_heads}H, {config.d_ff}ff")
        print(f"   MoE: {config.num_experts} experts, top-{config.expert_top_k} routing")
        print(f"   MHLA latents: {config.num_latents}")
        print(f"   Training: {config.max_steps} steps, batch size {config.batch_size}")
        print(f"   Data: {config.max_tokens:,} tokens, seq_len {config.max_seq_len}")
        
        results = compare_combinations(config, train_loader, val_loader)
    else:
        # Train single model
        print(f"\n{'='*60}")
        print(f"🧪 TRAINING: MoE + {config.attention_type.upper()} + {config.activation_type.upper()}")
        print(f"{'='*60}")
        ...
        print(f"\n🎯 {config.attention_type.upper()}-{config.activation_type.upper()} Model Results:")
