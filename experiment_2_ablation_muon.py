...
class Muon(torch.optim.Optimizer):
    ...
    def __init__(self, params, lr=0.02, momentum=0.95, nesterov=True, ns_steps=5, use_newton_schulz=True, use_momentum=True):
        defaults = dict(lr=lr, momentum=momentum, nesterov=nesterov, ns_steps=ns_steps, 
                       use_newton_schulz=use_newton_schulz, use_momentum=use_momentum)
    ...
    def step(self):
        for group in self.param_groups:
            for p in group["params"]:
                ...
                # Apply Newton-Schulz orthogonalization if enabled
...
def setup_muon_optimizer(model: nn.Module, config: MoEModelConfig, variant="full"):
    """Setup Muon optimizer with different ablation variants"""
    muon_params = []
    adamw_params = []
    ...
    # Configure Muon based on ablation variant
    if variant == "full":
        muon_optimizer = Muon(muon_params, lr=config.muon_lr, momentum=0.95, 
                             use_newton_schulz=True, use_momentum=True)
    elif variant == "no_newton_schulz":
        muon_optimizer = Muon(muon_params, lr=config.muon_lr, momentum=0.95,
                             use_newton_schulz=False, use_momentum=True)
    elif variant == "no_momentum":
        muon_optimizer = Muon(muon_params, lr=config.muon_lr, momentum=0.95,
                             use_newton_schulz=True, use_momentum=False)
    elif variant == "no_both":
        muon_optimizer = Muon(muon_params, lr=config.muon_lr, momentum=0.95,
                             use_newton_schulz=False, use_momentum=False)
    else:
