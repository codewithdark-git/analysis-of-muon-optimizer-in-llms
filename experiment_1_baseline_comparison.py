...
def setup_adamw_optimizer(model: nn.Module, config: MoEModelConfig):
    """Setup pure AdamW optimizer for baseline comparison"""
    all_params = [p for p in model.parameters() if p.requires_grad]
    print(f"  AdamW parameters: {sum(p.numel() for p in all_params):,}")
    
    optimizer = torch.optim.AdamW(all_params, lr=config.muon_lr, weight_decay=config.weight_decay)
    return [optimizer]


def train_moe_model(config: MoEModelConfig, train_loader: DataLoader, val_loader: DataLoader, optimizer_type="muon"):
    """Train the MoE model with specified optimizer"""
    optimizer_name = "Muon (Hybrid)" if optimizer_type == "muon" else "AdamW (Baseline)"
    print(f"\n🚀 Training MoE model with {config.num_experts} experts (top-{config.expert_top_k}) using {optimizer_name}")
    ...
    if optimizer_type == "muon":
        ...
        optimizers = setup_adamw_optimizer(model, config)
...
if __name__ == "__main__":
    ...
    print(f"\n{'='*80}")
    print(f"🧪 EXPERIMENT 1: Muon vs AdamW Baseline Comparison")
    print(f"{'='*80}")
    ...
    print(f"\n{'='*40}")
    print(f"🚀 Training with Muon (Hybrid) Optimizer")
    print(f"{'='*40}")
    start_time = time.time()
    model_muon, final_metrics_muon = train_moe_model(config, train_loader, val_loader, "muon")
    muon_time = time.time() - start_time
    results['muon'] = {'metrics': final_metrics_muon, 'time': muon_time}
    
    print(f"\n🎯 Muon Results:")
    print(f"⏱️ Training time: {muon_time/60:.1f} minutes")
    print(f"🏆 Final Results:")
    print(f"   Validation Loss: {final_metrics_muon['val_loss']:.4f}")
    print(f"   Validation Accuracy: {final_metrics_muon['val_accuracy']:.4f}")
    print(f"   Validation Perplexity: {final_metrics_muon['val_perplexity']:.2f}")

    # Train with AdamW optimizer
    print(f"\n{'='*40}")
    print(f"🚀 Training with AdamW (Baseline) Optimizer")
    print(f"{'='*40}")
    start_time = time.time()
    model_adamw, final_metrics_adamw = train_moe_model(config, train_loader, val_loader, "adamw")
    adamw_time = time.time() - start_time
    results['adamw'] = {'metrics': final_metrics_adamw, 'time': adamw_time}
    
    print(f"\n🎯 AdamW Results:")
    print(f"⏱️ Training time: {adamw_time/60:.1f} minutes")
    print(f"🏆 Final Results:")
    print(f"   Validation Loss: {final_metrics_adamw['val_loss']:.4f}")
    print(f"   Validation Accuracy: {final_metrics_adamw['val_accuracy']:.4f}")
    print(f"   Validation Perplexity: {final_metrics_adamw['val_perplexity']:.2f}")

    # Comparison Summary
    print(f"\n{'='*80}")
    print(f"📊 COMPARISON SUMMARY")
    print(f"{'='*80}")
    print(f"{'Metric':<20} {'Muon':<15} {'AdamW':<15} {'Difference':<15}")
    print(f"{'-'*65}")
    print(f"{'Val Loss':<20} {final_metrics_muon['val_loss']:<15.4f} {final_metrics_adamw['val_loss']:<15.4f} {final_metrics_muon['val_loss'] - final_metrics_adamw['val_loss']:<15.4f}")
    print(f"{'Val Accuracy':<20} {final_metrics_muon['val_accuracy']:<15.4f} {final_metrics_adamw['val_accuracy']:<15.4f} {final_metrics_muon['val_accuracy'] - final_metrics_adamw['val_accuracy']:<15.4f}")
    print(f"{'Val Perplexity':<20} {final_metrics_muon['val_perplexity']:<15.2f} {final_metrics_adamw['val_perplexity']:<15.2f} {final_metrics_muon['val_perplexity'] - final_metrics_adamw['val_perplexity']:<15.2f}")
    print(f"{'Training Time (min)':<20} {muon_time/60:<15.1f} {adamw_time/60:<15.1f} {(muon_time - adamw_time)/60:<15.1f}")
