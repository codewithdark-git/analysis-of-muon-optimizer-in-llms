...
class MoEMinimalLLM(nn.Module):
    ...
    def __init__(self, config: MoEModelConfig):
        ...
        # Transformer blocks with MoE and configurable attention
...
def train_moe_model(config: MoEModelConfig, train_loader: DataLoader, val_loader: DataLoader):
    ...
    model.train()
    step = 0
    pbar = tqdm(total=config.max_steps, desc=f"Training MoE+{config.attention_type.upper()}")
...
def compare_attention_mechanisms(config: MoEModelConfig, train_loader: DataLoader, val_loader: DataLoader):
    """Compare MHSA vs MHLA performance"""
    results = {}
    
    for attention_type in ["mhsa", "mhla"]:
        print(f"\n{'='*60}")
        print(f"🧪 TRAINING: MoE + {attention_type.upper()}")
        print(f"{'='*60}")
        
        # Create config for this attention type
    ...
    # Print comparison
    print(f"\n{'='*60}")
    print(f"📈 COMPARISON: MHSA vs MHLA")
    print(f"{'='*60}")
    
    mhsa_results = results["mhsa"]
    mhla_results = results["mhla"]
    
    print(f"Training Time:")
    print(f"  MHSA: {mhsa_results['training_time']/60:.1f} minutes")
    print(f"  MHLA: {mhla_results['training_time']/60:.1f} minutes")
    print(f"  Speedup: {mhsa_results['training_time']/mhla_results['training_time']:.2f}x")
    
    print(f"\nValidation Loss:")
    print(f"  MHSA: {mhsa_results['metrics']['val_loss']:.4f}")
    print(f"  MHLA: {mhla_results['metrics']['val_loss']:.4f}")
    print(f"  Difference: {mhla_results['metrics']['val_loss'] - mhsa_results['metrics']['val_loss']:+.4f}")
    
    print(f"\nValidation Perplexity:")
    print(f"  MHSA: {mhsa_results['metrics']['val_perplexity']:.2f}")
    print(f"  MHLA: {mhla_results['metrics']['val_perplexity']:.2f}")
    print(f"  Difference: {mhla_results['metrics']['val_perplexity'] - mhsa_results['metrics']['val_perplexity']:+.2f}")
    
    # Parameter comparison
    mhsa_params = sum(p.numel() for p in mhsa_results['model'].parameters())
    mhla_params = sum(p.numel() for p in mhla_results['model'].parameters())
    
    print(f"\nModel Parameters:")
    print(f"  MHSA: {mhsa_params:,}")
    print(f"  MHLA: {mhla_params:,}")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Train MoE model with configurable attention")
    ...
    if args.attention == "both":
        # Compare both attention mechanisms
        print(f"\n📋 Comparing both attention mechanisms:")
        ...
        results = compare_attention_mechanisms(config, train_loader, val_loader)
    else:
        # Train single model
        print(f"\n{'='*60}")
        print(f"🧪 TRAINING: MoE + {config.attention_type.upper()}")
        print(f"{'='*60}")
        ...
        print(f"\n🎯 {config.attention_type.upper()} Model Results:")
